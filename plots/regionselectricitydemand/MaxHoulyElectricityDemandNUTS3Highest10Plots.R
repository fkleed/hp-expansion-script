...
highest_regions_electricity_demand_sh_and_hw_reference <-
  regions_electricity_demand_reference %>%
  slice_max(`Electricity demand`, n = 10) %>%
...
highest_regions_electricity_demand_sh_and_hw <-
  highest_regions_electricity_demand_sh_and_hw_reference %>%
  rbind(highest_regions_electricity_demand_sh_and_hw_cold) %>%
  rbind(highest_regions_electricity_demand_sh_and_hw_hot)

highest_regions_maximum_hourly_electricity_demand_sh_and_hw_plot <-
  ggplot(
    data = highest_regions_electricity_demand_sh_and_hw,
    aes(
...
highest_regions_electricity_demand_sh_only <-
  highest_regions_electricity_demand_sh_only_reference %>%
  rbind(highest_regions_electricity_demand_sh_only_cold) %>%
  rbind(highest_regions_electricity_demand_sh_only_hot)

highest_regions_maximum_hourly_electricity_demand_sh_only_plot <-
  ggplot(
    data = highest_regions_electricity_demand_sh_only,
    aes(
      x = `Electricity demand` / 1000,
      y = reorder(NUTS3Name,-`Electricity demand`)
    )
...
# Save the plots
ggsave(
  "plots/output/regionselectricitydemand/maxhourlyelectricitydemandnuts3highest10/highest_regions_maximum_hourly_electricity_demand_sh_and_hw_plot.png",
  highest_regions_maximum_hourly_electricity_demand_sh_and_hw_plot,
  width = 30,
  units = "cm"
)
...
ggsave(
  "plots/output/regionselectricitydemand/maxhourlyelectricitydemandnuts3highest10/highest_regions_maximum_hourly_electricity_demand_sh_only_plot.png",
  highest_regions_maximum_hourly_electricity_demand_sh_only_plot,
  width = 30,
  units = "cm"
)
